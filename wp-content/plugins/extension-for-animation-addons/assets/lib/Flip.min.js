...
! function(t, e) {
    ...
    function ja(e, i, n) {
        ...
        for (a = "none" === c("display"), e.isVisible && !a || (a && (fa(e, ["display"]).display = i.display), e.matrix = i.matrix, e.width = l = e.width || i.width, e.height = p = e.height || i.height), fa(e, j), s = window.getComputedStyle(o); d--;) f[j[d]] = s[j[d]];
    ...
    function Aa(t, e, r, i) {
        ...
        for (s in e.idLookup) d = e.alt[s] ? va(e, t, s) : e.idLookup[s], u = d.element, n = t.idLookup[s], !t.alt[s] || u !== n.element || !t.alt[s].isVisible && d.isVisible || (n = t.alt[s]), n ? (p = {
            ...
            sd: -p.sd,
            swap: p
        })), u._flip = n.element._flip = tt ? tt.timeline : j) : d.isVisible && (H.push({
            t: u,
            b: _(d, {
                isVisible: 1
            }),
            ...
            sd: 0,
            entering: 1
        }), u._flip = tt ? tt.timeline : j);
        Y && (st[Y] || ba(Y)).forEach(function(e) {
            return z[e] = function(t) {
                return H[t].a.props[e]
            }
        }), H.finalStates = f = [], m = function run() {
            for (ea(H), za(!0), l = 0; l < H.length; l++) p = H[l], g = p.a, v = p.b, !D || g.isDifferent(v) || p.entering ? (u = p.t, !I || p.sd < 0 || !l || (g.matrix = getGlobalMatrix(u, !1, !1, !0)), v.isVisible && g.isVisible ? (p.sd < 0 ? (h = new pt(u, Y, t.simple), ot(h, g, T, 0, 0, h), h.matrix = getGlobalMatrix(u, !1, !1, !0), h.css = p.b.css, p.a = g = h, A && (u.style.opacity = J ? v.opacity : g.opacity), O && q.push(u)) : 0 < p.sd && A && (u.style.opacity = J ? g.opacity - v.opacity : "0"), ot(g, v, T, Y)) : v.isVisible !== g.isVisible && (v.isVisible ? g.isVisible || (v.css = g.css, G.push(v), H.splice(l--, 1), S && I && ot(g, v, T, Y)) : (g.isVisible && W.push(g), H.splice(l--, 1))), T || (u.style.maxWidth = Math.max(g.width, v.width) + "px", u.style.maxHeight = Math.max(g.height, v.height) + "px", u.style.minWidth = Math.min(g.width, v.width) + "px", u.style.minHeight = Math.min(g.height, v.height) + "px"), I && L && u.classList.add(L)) : H.splice(l--, 1), f.push(g);
            var e;
            ...
            if (it.forEach(function(t) {
                    return r[t] && j.eventCallback(t, r[t], r[t + "Params"])
                }), C && c.length)
                for (s in R = _(z, rt), "scale" in C && (C.scaleX = C.scaleY = C.scale, delete C.scale), C)(a = _(C[s], at))[s] = z[s], !("duration" in a) && "duration" in z && (a.duration = z.duration), a.stagger = z.stagger, U.call(j, c, a, 0), delete R[s];
            (c.length || G.length || W.length) && (L && j.add(function() {
                return X(e, L, j._zTime < 0 ? "remove" : "add")
            }, 0) && !E && X(e, L, "add"), c.length && U.call(j, c, R, 0)), sa(b, W, j), sa(w, G, j);
            var n = tt && tt.timeline;
            n && (n.add(j, 0), tt._final.push(function() {
                return ha(H, !x)
    ...
    var I, Q, tt, s, o, P, T, l, n, h = 1,
        ...
        st = {},
        j = "paddingTop,paddingRight,paddingBottom,paddingLeft,gridArea,transition".split(","),
        R = function _parseElementState(t, e, i, n) {
            return t instanceof pt ? t : t instanceof ut ? function _findElStateInState(t, e) {
                return e && t.idLookup[R(e).id] || t.elementStates[0]
    ...
    function Flip() {}
    ht.version = "3.11.5", "undefined" != typeof window && window.gsap && window.gsap.registerPlugin(ht), e.Flip = ht, e.default = ht;
